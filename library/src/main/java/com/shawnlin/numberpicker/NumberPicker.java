...
import com.maizer.compater.ResourceCompater;
...
public class NumberPicker extends LinearLayout {
	...
	public static final String TAG = NumberPicker.class.getCanonicalName();
	...
	private boolean isScrollPrecise;
	...
	private int mMaxFlingVelocityAdjust = SELECTOR_MAX_FLING_VELOCITY_ADJUSTMENT;
	...
	public NumberPicker(Context context, AttributeSet attrs, int defStyle) {
		...
		mSelectionDivider = ResourceCompater.getDrawable(context, R.drawable.np_numberpicker_selection_divider);

		mSelectionDividerColor = attributesArray.getColor(R.styleable.NumberPicker_np_dividerColor,
		...
		mSelectionDividersDistance = attributesArray.getDimensionPixelSize(R.styleable.NumberPicker_np_dividerDistance,
				defSelectionDividerDistance);

		final int defSelectionDividerThickness = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP,
				UNSCALED_DEFAULT_SELECTION_DIVIDER_THICKNESS, getResources().getDisplayMetrics());
		mSelectionDividerThickness = attributesArray.getDimensionPixelSize(R.styleable.NumberPicker_np_dividerThickness,
				defSelectionDividerThickness);
		...
		mSelectedTextColor = attributesArray.getColor(R.styleable.NumberPicker_np_selectedTextColor,
				mSelectedTextColor);
		...
		mSelectedTextSize = attributesArray.getDimension(R.styleable.NumberPicker_np_selectedTextSize,
		...
		isScrollPrecise = attributesArray.getBoolean(R.styleable.NumberPicker_np_precise, false);
		...
		mSelectedText = (EditText) inflater.inflate(R.layout.number_picker_with_selector_wheel, this, false);
		...
		mWrapSelectorWheel = attributesArray.getBoolean(R.styleable.NumberPicker_np_wrapSelectorWheel,
				mWrapSelectorWheel);
		...
		mMaximumFlingVelocity = configuration.getScaledMaximumFlingVelocity() / mMaxFlingVelocityAdjust;
	...
	/**
	 * set max fling velocity scale.
	 * 
	 * @param scale
	 */
	public void setSelectorMaxFlingVelocityAdjustMent(int scale) {
		ViewConfiguration configuration = ViewConfiguration.get(getContext());
		mMaxFlingVelocityAdjust = scale;
		mMaximumFlingVelocity = configuration.getScaledMaximumFlingVelocity() / mMaxFlingVelocityAdjust;
	}
	...
	protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
		...
		if (changed) {
			...
			if (isHorizontalMode()) {
				...
				mRightOfSelectionDividerRight = mLeftOfSelectionDividerLeft + 2 * mSelectionDividerThickness
						+ mSelectionDividersDistance;
			} else {
				mTopSelectionDividerTop = (getHeight() - mSelectionDividersDistance) / 2 - mSelectionDividerThickness;
				mBottomSelectionDividerBottom = mTopSelectionDividerTop + 2 * mSelectionDividerThickness
						+ mSelectionDividersDistance;
			}
		}
	}
		...
		case MotionEvent.ACTION_UP: {
			...
			if (isHorizontalMode()) {
				...
				if (Math.abs(initialVelocity) > mMinimumFlingVelocity) {
					...
					if (deltaMoveX <= mTouchSlop) { // && deltaTime <
	...
	public void scrollBy(int x, int y) {
		...
		while (mCurrentScrollOffset > mSelectorElementSize) {
		...
		Log.e(TAG, "" + selectorIndices[selectorIndices.length - 1]);
	 ...
	 * @param enable
	 */
	public void setPreciseScrollEnable(boolean enable) {
		isScrollPrecise = enable;
	}
	...
	public void setDisplayedValues(String[] displayedValues) {
		...
		if (mDisplayedValues != null) {
			...
			mSelectedText.setRawInputType(InputType.TYPE_CLASS_TEXT | InputType.TYPE_TEXT_FLAG_NO_SUGGESTIONS);
		} else {
	...
	private void initializeSelectorWheel() {
		initializeSelectorWheelIndices();
		float textGapCount = mWheelItemCount;
		int totalTextSize = mWheelItemCount * (int) mTextSize;
	...
	private void fling(int velocity) {
		if (isHorizontalMode()) {
			...
			if (velocity > 0) {
				...
				if (isScrollPrecise) {
					mFlingScroller.setFinalX(
							mFlingScroller.getFinalX() / mSelectorElementSize * mSelectorElementSize + getDeltaValue());
				}
			} else {
				mFlingScroller.fling(Integer.MAX_VALUE, 0, velocity, 0, 0, Integer.MAX_VALUE, 0, 0);
				if (isScrollPrecise) {
					mFlingScroller.setFinalX(Integer.MAX_VALUE - (Integer.MAX_VALUE - mFlingScroller.getFinalX())
							/ mSelectorElementSize * mSelectorElementSize + getDeltaValue());
				}
			}
		} else {
			mPreviousScrollerY = 0;
			if (velocity > 0) {
				...
				if (isScrollPrecise) {
					int finalY = mFlingScroller.getFinalY();
					if (finalY < mSelectorElementSize) {
						finalY = mSelectorElementSize;
					}
					mFlingScroller.setFinalY(finalY / mSelectorElementSize * mSelectorElementSize + getDeltaValue());
				}
			} else {
				mFlingScroller.fling(0, Integer.MAX_VALUE, 0, velocity, 0, 0, 0, Integer.MAX_VALUE);
				if (isScrollPrecise) {
					int finalY = mFlingScroller.getFinalY();
					if (Integer.MAX_VALUE - finalY < mSelectorElementSize) {
						finalY = Integer.MAX_VALUE - mSelectorElementSize;
					}
					mFlingScroller.setFinalY(Integer.MAX_VALUE
							- (Integer.MAX_VALUE - finalY) / mSelectorElementSize * mSelectorElementSize
							+ getDeltaValue());
				}
			}
		}
	 ...
	 */
	private String ensureCachedScrollSelectorValue(int selectorIndex) {
		SparseArray<String> cache = mSelectorIndexToStringCache;
		String scrollSelectorValue = cache.get(selectorIndex);
		if (scrollSelectorValue != null) {
			return scrollSelectorValue;
		}
	...
	class InputTextFilter extends NumberKeyListener {
		...
		// XXX This doesn't allow for range limits when controlled by a soft
		// input method!
	 ...
	 */
	private boolean ensureScrollWheelAdjusted(boolean checkPrecise) {
		if (checkPrecise && isScrollPrecise) {
			return true;
		}
		...
		if (delta != 0) {
			if (isHorizontalMode()) {
				mPreviousScrollerX = 0;
				mFlingScroller.startScroll(0, 0, delta, 0, SELECTOR_ADJUSTMENT_DURATION_MILLIS);
			} else {
				mPreviousScrollerY = 0;
				mFlingScroller.startScroll(0, 0, 0, delta, SELECTOR_ADJUSTMENT_DURATION_MILLIS);
			}
			invalidate();
			return true;
		}
	...
	private int getDeltaValue() {
		...
		if (delta != 0) {
			if (Math.abs(delta) > mSelectorElementSize >> 1) {
	...
	public void setDividerColorResource(int colorId) {
		setDividerColor(ResourceCompater.getColor(mContext, colorId));
	}
	...
	public void setWheelItemCount(final int count) {
		...
		mSelectorIndices = new int[mWheelItemCount + 1];
	}
	...
	public void setSelectedTextColorResource(int colorId) {
		setSelectedTextColor(ResourceCompater.getColor(mContext, colorId));
	}
	...
	public void setTextColorResource(int colorId) {
		setTextColor(ResourceCompater.getColor(mContext, colorId));
	}
